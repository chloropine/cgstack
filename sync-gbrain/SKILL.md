...
description: |
  ...
  guidance in AGENTS.md. Wraps the cgstack-gbrain-sync orchestrator with
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `~/.cgstack/` indexing (the existing `cgstack-gbrain-source-wireup`
...
`~/.codex/skills/cgstack/bin/cgstack-gbrain-sync.ts`.
...
```bash
~/.codex/skills/cgstack/bin/cgstack-gbrain-detect 2>/dev/null
```
...
also runs locally** in local-stdio MCP mode — `cgstack-memory-ingest` shells
...
`~/.cgstack/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `cgstack-brain-sync`
...
Also check the per-repo trust policy. If `cgstack-gbrain-repo-policy get` for
...
```bash
bun run ~/.codex/skills/cgstack/bin/cgstack-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.cgstack/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.cgstack/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.cgstack/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- cgstack-gbrain-search-guidance:start -->` through
`<!-- cgstack-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <cgstack-code-{repo_slug}> (page_count=<N>)
  ~/.cgstack source. OK   <cgstack-brain-{user}> (page_count=<N>) — managed by $setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.cgstack/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.cgstack/.brain-allowlist`
(which is for `~/.cgstack/` brain-sync only). On a different Mac with a synced
