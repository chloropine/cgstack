// cgstack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/cgstack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.cgstack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        cgstack_version: String(event.cgstack_version).slice(0, 20),
