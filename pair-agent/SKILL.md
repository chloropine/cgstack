...
version: 0.1.0
description: |
  Pair a remote Codex session with your browser. One command generates a setup key and
  prints instructions another Codex session can follow to connect. Works only with
  OpenAI Codex. The remote Codex session
  gets its own tab with scoped access (read+write by default, admin on request).
  Use when asked to "pair agent", "connect agent", "share browser", "remote browser",
  "let another Codex session use my browser", or "give browser access". (cgstack)
  Voice triggers (speech-to-text aliases): "pair agent", "connect agent", "share my browser", "remote browser access".
triggers:
  - pair with agent
  - connect remote Codex session
  - share my browser
allowed-tools:
  - Bash
  - Read
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"pair-agent","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"pair-agent","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# $pair-agent — Share Your Browser With Another Codex Session

You're sitting in Codex with a browser running. You also have another Codex
session open. You want that session to browse the web using YOUR browser. This
skill makes that happen.

## How it works

Your cgstack browser runs a local HTTP server. This skill creates a one-time setup key,
prints a block of instructions, and you paste those instructions into the other Codex session.
The other Codex session exchanges the key for a session token, creates its own tab,
and starts browsing. Each Codex session gets its own tab. They can't mess with each
other's tabs.
...
**Same machine:** If the other Codex session is on the same machine,
locally), you can skip the copy-paste ceremony and write the credentials directly to
the Codex config directory.

**Remote:** If the other Codex session is on a different machine, you need an ngrok tunnel.
...
> Is the other Codex session running on this same machine, or on a different machine/server?
>
> **Same machine** skips the copy-paste ceremony. Credentials are written directly to
> the Codex config directory. No tunnel needed.
>
...
"Done. Codex can now use your browser. It will read credentials from the
...
response so the user can copy-paste it into their other Codex session. Do NOT summarize it,
...
Then tell the user:
"Copy the block above and paste it into your other Codex session's chat. The setup key
...
Then retry `$B pair-agent --client codex`.

**If ngrok is NOT installed:** Walk the user through installation:

Tell the user:
"To connect a remote Codex session, we need ngrok (a tunnel that exposes your local
...
After the user pastes the instructions into the other Codex session, wait a moment then check:
...
"The remote Codex session is connected and has its own tab. You'll see its activity in the
side panel if you have CGStack Browser open."

## What the remote Codex session can do
...
**"Tab not owned by your agent"** — The remote Codex session tried to interact with a tab
...
to `~/.codex/skills/cgstack/browse-remote.json`.
