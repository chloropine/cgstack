...
description: |
  ...
  what it could look like. (cgstack)
triggers:
...
gbrain:
  ...
  context_queries:
    - id: prior-approved-variants
      kind: filesystem
      glob: "~/.cgstack/projects/{repo_slug}/designs/*/approved.json"
      sort: mtime_desc
      limit: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"design-shotgun","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_PREV=$(find ~/.cgstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls ~/.cgstack/projects/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
**Persistent taste profile (v1 schema at `~/.cgstack/projects/$SLUG/taste-profile.json`):**
...
the legacy approved.json aggregate — `~/.codex/skills/cgstack/bin/cgstack-taste-update`
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.cgstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
variant, call `~/.codex/skills/cgstack/bin/cgstack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.codex/skills/cgstack/bin/cgstack-taste-update rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.cgstack/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.cgstack/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
