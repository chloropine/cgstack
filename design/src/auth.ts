 ...
 *
 * Resolution order:
 * 1. ~/.cgstack/openai.json → { "api_key": "sk-..." }
...
const CONFIG_PATH = path.join(process.env.HOME || "~", ".cgstack", "openai.json");

export function resolveApiKey(): string | null {
  // 1. Check ~/.cgstack/openai.json
  try {
...
/**
 * Save an API key to ~/.cgstack/openai.json with 0600 permissions.
 */
...
export function requireApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("  or save to ~/.cgstack/openai.json: { \"api_key\": \"sk-...\" }");
