...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"benchmark","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
mkdir -p .cgstack/benchmark-reports
mkdir -p .cgstack/benchmark-reports/baselines
```
...
Write to `.cgstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.cgstack/benchmark-reports/{date}-benchmark.md` and `.cgstack/benchmark-reports/{date}-benchmark.json`.
