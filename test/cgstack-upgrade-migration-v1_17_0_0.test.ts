...
const MIGRATION = path.join(ROOT, 'cgstack-upgrade', 'migrations', 'v1.17.0.0.sh');
...
function makeFakeStubs(opts: {
  configValue?: string; // value cgstack-config returns for gbrain_sync_mode
  configMissing?: boolean; // cgstack-config binary itself missing (test edge)
  wireupMissing?: boolean; // wireup binary missing
  wireupExitCode?: number;
}) {
  const skillsBin = path.join(tmpHome, '.codex', 'skills', 'cgstack', 'bin');
  fs.mkdirSync(skillsBin, { recursive: true });

  if (!opts.configMissing) {
    const cfg = `#!/bin/bash
echo "cgstack-config $@" >> "${stubLog}"
[ "$1" = "get" ] && [ "$2" = "gbrain_sync_mode" ] && echo "${opts.configValue ?? ''}"
exit 0
`;
    fs.writeFileSync(path.join(skillsBin, 'cgstack-config'), cfg, { mode: 0o755 });
  }

  if (!opts.wireupMissing) {
    const wu = `#!/bin/bash
echo "cgstack-gbrain-source-wireup $@" >> "${stubLog}"
exit ${opts.wireupExitCode ?? 0}
`;
    fs.writeFileSync(path.join(skillsBin, 'cgstack-gbrain-source-wireup'), wu, { mode: 0o755 });
  }
}

function makeBrainGitRepo() {
  const cgstackHome = path.join(tmpHome, '.cgstack');
  fs.mkdirSync(path.join(cgstackHome, '.git'), { recursive: true });
}
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-migration-test-'));
...
describe('migrations/v1.17.0.0.sh', () => {
  ...
  test('gbrain_sync_mode = off: exit 0 silently (no helper invoked)', () => {
    ...
    expect(calls.some((c) => c.startsWith('cgstack-gbrain-source-wireup'))).toBe(false);
  });
  ...
  test('no ~/.cgstack/.git: exit 0 silently (no brain-sync configured)', () => {
    makeFakeStubs({ configValue: 'full' });
    // Do NOT call makeBrainGitRepo() — no .cgstack/.git directory exists
    const r = run();
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('cgstack-gbrain-source-wireup'))).toBe(false);
  ...
  test('happy path: invokes the helper', () => {
    ...
    expect(r.status).toBe(0);
    const calls = stubCalls();
    expect(calls.some((c) => c.startsWith('cgstack-gbrain-source-wireup'))).toBe(true);
    // Note: migration invokes WITHOUT --strict (benign-skip semantics for batch upgrade)
    const helperCall = calls.find((c) => c.startsWith('cgstack-gbrain-source-wireup'));
