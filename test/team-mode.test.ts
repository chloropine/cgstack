...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'cgstack-settings-hook');
const SESSION_UPDATE = path.join(ROOT, 'bin', 'cgstack-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'cgstack-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-team-test-'));
}
...
describe('cgstack-settings-hook', () => {
  let tmpDir: string;
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/cgstack-session-update`, {
      env: { CGSTACK_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/cgstack-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/cgstack-session-update`, {
      env: { CGSTACK_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/cgstack-session-update`, {
      env: { CGSTACK_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/cgstack-session-update`, {
      env: { CGSTACK_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/cgstack-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/cgstack-session-update`, {
      env: { CGSTACK_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('cgstack-session-update', () => {
  let tmpDir: string;
  let cgstackDir: string;
  let stateDir: string;

  beforeEach(() => {
    tmpDir = mkTmpDir();
    cgstackDir = path.join(tmpDir, 'cgstack');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(cgstackDir, { recursive: true });
    fs.mkdirSync(stateDir, { recursive: true });

    // Init a git repo to pass the .git guard
    execSync('git init', { cwd: cgstackDir });
    execSync('git commit --allow-empty -m "init"', { cwd: cgstackDir });
    fs.writeFileSync(path.join(cgstackDir, 'VERSION'), '0.1.0');

    // Create a minimal cgstack-config that returns auto_upgrade=true
    const binDir = path.join(cgstackDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'cgstack-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'cgstack-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(cgstackDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { CGSTACK_DIR: cgstackDir, CGSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override cgstack-config to return false
    fs.writeFileSync(path.join(cgstackDir, 'bin', 'cgstack-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { CGSTACK_DIR: cgstackDir, CGSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { CGSTACK_DIR: '/nonexistent/path', CGSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('cgstack-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates AGENTS.md with recommended section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const codex = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    expect(codex).toContain('## cgstack (recommended)');
    expect(codex).toContain('./setup --team');
  });
  ...
  test('required: creates AGENTS.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const codex = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    expect(codex).toContain('## cgstack (REQUIRED');
    expect(codex).toContain('CGSTACK_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.codex', 'hooks', 'check-cgstack.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: cgstack is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-cgstack');
  });
  ...
  test('idempotent: running twice does not duplicate AGENTS.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const codex = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    const matches = codex.match(/## cgstack/g);
  ...
  test('removes vendored copy when present', () => {
    ...
    execSync('git commit -m "add vendored cgstack"', { cwd: tmpDir });

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored cgstack copy');
    ...
    expect(gitignore).toContain('.agents/skills/cgstack/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored cgstack copy');
  });
  ...
  test('skips when .agents/skills/cgstack is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'cgstack'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored cgstack copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'cgstack')).isSymbolicLink()).toBe(true);
...
describe('setup --team / --no-team / -q', () => {
  ...
  test(
    ...
    () => {
      ...
      // The key test is that the "Skill naming:" prompt and "cgstack ready" messages are suppressed
      expect(result.stdout).not.toContain('Skill naming:');
      expect(result.stdout).not.toContain('cgstack ready');
    },
    180_000,
  );
