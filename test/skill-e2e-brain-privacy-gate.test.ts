 ...
 * on PATH), runs a cheap cgstack skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('cgstack skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh CGSTACK_HOME with artifacts_sync_mode_prompted=false.
    const cgstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-cgstack-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-bin-'));

    // Seed the config so the gate's condition passes.
    fs.writeFileSync(
      path.join(cgstackHome, 'config.yaml'),
    ...
    // don't inherit them.
    const origGstackHome = process.env.CGSTACK_HOME;
    ...
    try {
      ...
      await runAgentSdkTest({
        ...
        // env object, its auth pipeline doesn't pick up OPENAI_API_KEY the
      ...
      if (origGstackHome === undefined) delete process.env.CGSTACK_HOME;
      else process.env.CGSTACK_HOME = origGstackHome;
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const cgstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
