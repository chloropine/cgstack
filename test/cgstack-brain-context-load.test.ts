...
const SCRIPT = join(import.meta.dir, "..", "bin", "cgstack-brain-context-load.ts");
...
describe("cgstack-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cgstack-brain-context-load");
...
describe("cgstack-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "cgstack-bcl-"));
...
describe("cgstack-brain-context-load — graceful gbrain absence", () => {
