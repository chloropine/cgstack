/**
 * Unit tests for lib/cgstack-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("canonicalizeRemote", () => {
  it("strips https scheme and .git suffix", () => {
    expect(canonicalizeRemote("https://github.com/chloropine/cgstack.git")).toBe("github.com/chloropine/cgstack");
  });

  it("normalizes git@host:path scp-style remotes", () => {
    expect(canonicalizeRemote("git@github.com:chloropine/cgstack.git")).toBe("github.com/chloropine/cgstack");
  });
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "cgstack-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.cgstack/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.CGSTACK_HOME;
    testHome = mkdtempSync(join(tmpdir(), "cgstack-test-home-"));
    process.env.CGSTACK_HOME = testHome;
  });

  afterAll(() => {
    if (savedHome === undefined) delete process.env.CGSTACK_HOME;
    else process.env.CGSTACK_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    ...
    testHome = mkdtempSync(join(tmpdir(), "cgstack-test-engine-"));
    testGbrainHome = mkdtempSync(join(tmpdir(), "cgstack-test-gbrain-"));
    process.env.CGSTACK_HOME = testHome;
  ...
  it("writes a cache file at ~/.cgstack/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("cgstack-memory-helpers.detectEngineTier");
  });
