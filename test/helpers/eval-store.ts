 ...
 * ~/.cgstack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.cgstack-dev', 'evals');

/**
 * Detect project-scoped eval dir via cgstack-slug.
 * Falls back to legacy ~/.cgstack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local cgstack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.agents/skills/cgstack/bin/cgstack-slug 2>/dev/null || ~/.codex/skills/cgstack/bin/cgstack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.cgstack', 'projects', slugMatch[1], 'evals');
...
export interface EvalTestEntry {
  ...
  // Model + timing diagnostics (added for GPT/GPT split)
  model?: string;                // e.g. 'gpt-5.4' or 'gpt-5.4'
 ...
 * `CGSTACK_BUDGET_RATIO` env var. New tests with no prior data are skipped.
 */
export function findBudgetRegressions(
  comparison: ComparisonResult,
  opts?: { ratioCap?: number; minPriorTools?: number; minPriorTurns?: number },
): BudgetRegression[] {
  const envRatio = Number(process.env.CGSTACK_BUDGET_RATIO);
...
export function assertNoBudgetRegression(
  ...
  const cap = opts?.ratioCap ?? (Number(process.env.CGSTACK_BUDGET_RATIO) || 2.0);
  ...
  throw new Error(
    ...
    lines.join('\n') +
    `\n(Override per run: CGSTACK_BUDGET_RATIO=<n>. ${comparison.before_file} vs ${comparison.after_file})`,
  );
}
