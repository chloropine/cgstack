/**
 * Unit tests for bin/cgstack-gbrain-sync.ts (Lane B).
 *
...
describe("cgstack-gbrain-sync CLI", () => {
  it("--help exits 0 with usage text", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cgstack-gbrain-sync");
  ...
  it("--dry-run with --code-only reports the code import preview only", () => {
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
    ...
    expect(r.stdout).not.toContain("cgstack-memory-ingest --probe");
    expect(r.stdout).not.toContain("cgstack-brain-sync --discover-new");
  ...
  it("--dry-run with all stages shows previews for all three", () => {
    ...
    const r = runScript(["--dry-run"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("would: gbrain sources add");
    expect(r.stdout).toContain("gbrain sync --strategy code");
    expect(r.stdout).toContain("would: cgstack-memory-ingest");
    expect(r.stdout).toContain("would: cgstack-brain-sync");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("--no-code skips the code import stage", () => {
    ...
    const r = runScript(["--dry-run", "--no-code"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
  ...
  it("dry-run derives a stable source id from the canonical git remote", () => {
    // The source id pattern is `cgstack-code-<canonicalized-remote>`. For this
    // repo (github.com/chloropine/cgstack), the slug should appear in the dry-run
    // preview line. We don't pin the exact slug — just verify the prefix +
    // that the preview command would target a source with id cgstack-code-*.
    const home = makeTestHome();
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add cgstack-code-[a-z0-9-]+/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source cgstack-code-[a-z0-9-]+/);
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("derived source ids are gbrain-valid (≤32 chars, alnum + interior hyphens, no dots) for any remote", () => {
    ...
    // For `github.com/<org>/<repo>`, the id was `cgstack-code-github.com-<org>-<repo>`,
    ...
    const cases = [
      ...
      "git@github.com:chloropine/cgstack.git",            // SCP-style remote
    ...
    for (const remote of cases) {
      ...
      const cgstackHome = join(home, ".cgstack");
      mkdirSync(cgstackHome, { recursive: true });
      const repo = mkdtempSync(join(tmpdir(), "cgstack-source-id-repo-"));
      ...
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, CGSTACK_HOME: cgstackHome },
      });
      ...
      expect(id.startsWith("cgstack-code-")).toBe(true);
  ...
  it("derives a gbrain-valid source id when the cwd repo has NO origin remote", () => {
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-no-origin-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("derives a gbrain-valid source id when the basename sanitizes to empty", () => {
    ...
    // returned "cgstack-code-" (invalid trailing hyphen) and was patched to
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const parent = mkdtempSync(join(tmpdir(), "cgstack-empty-base-"));
  ...
  it("derives distinct source ids for the same absolute path on different hosts", () => {
    ...
    // key keeps them distinct. `CGSTACK_HOSTNAME` env var is the test-only knob;
    // production uses `os.hostname()`.
    const home = makeTestHome();
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-host-collide-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    const bindir = mkdtempSync(join(tmpdir(), "cgstack-host-collide-bin-"));
    ...
    const runAs = (host: string) =>
      spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, CGSTACK_HOME: cgstackHome, CGSTACK_HOSTNAME: host, PATH },
      });
  ...
  it("dry-run does NOT acquire the lock file (lock is for write paths only)", () => {
    ...
    const lockPath = join(cgstackHome, ".sync-gbrain.lock");
  ...
  it("writes a state file with schema_version: 1 after a non-dry run", () => {
    ...
    const statePath = join(cgstackHome, ".gbrain-sync-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("cgstack-gbrain-sync");
  ...
  it("records stage results in state file", () => {
    ...
    const state = JSON.parse(readFileSync(join(cgstackHome, ".gbrain-sync-state.json"), "utf-8"));
  ...
  it("brain-sync stage resolves the sibling binary, not a HOME-rooted path", () => {
    ...
    // ~/.codex/skills/cgstack/bin/cgstack-brain-sync, which silently no-op'd
    // on Codex installs and dev workspaces with the misleading summary
    // "skipped (cgstack-brain-sync not installed)". Post-fix it resolves
    ...
    const r = runScript(
      ...
      { HOME: home, CGSTACK_HOME: cgstackHome },
    );
    ...
    expect(combined).not.toContain("skipped (cgstack-brain-sync not installed)");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("worktree-aware source ID: two worktrees of the same repo get DIFFERENT ids", () => {
    ...
    // ID was slug-only so both worktrees collapsed onto `cgstack-code-<slug>` and
    ...
    const repoA = mkdtempSync(join(tmpdir(), "cgstack-worktree-a-"));
    const repoB = mkdtempSync(join(tmpdir(), "cgstack-worktree-b-"));
    ...
    expect(idA).not.toBe(idB);
    expect(idA.startsWith("cgstack-code-")).toBe(true);
    expect(idB.startsWith("cgstack-code-")).toBe(true);
  ...
  it("worktree-aware source ID: same path produces the same id across runs (deterministic)", () => {
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-worktree-stable-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", remote], { cwd: repo });
  ...
  it("dry-run preview includes legacy-source removal + attach (post-codex-review hardening)", () => {
    // Codex adversarial flagged: pre-pathhash `cgstack-code-<slug>` sources stay
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-legacy-cleanup-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/chloropine/cgstack.git"], { cwd: repo });
    ...
    // remove the legacy source via `gbrain sources remove cgstack-code-<slug>
    ...
    expect(r.stdout).toMatch(/gbrain sources add cgstack-code-/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source cgstack-code-/);
    expect(r.stdout).toMatch(/gbrain sources attach cgstack-code-/);
  ...
  it("dry-run preview includes the `sources attach` step (kubectl-style CWD pin)", () => {
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-attach-preview-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    spawnSync("git", ["remote", "add", "origin", "https://github.com/chloropine/cgstack.git"], { cwd: repo });
...
// creating a data-loss window. See bin/cgstack-gbrain-sync.ts and the
...
describe("derivePathOnlyHashLegacyId", () => {
  it("returns the pre-#1468 form (path-only sha1, no hostname)", () => {
    ...
    // legacy id regardless of $CGSTACK_HOSTNAME, because the pre-#1468 hash
    // didn't include hostname.
    const repo = mkdtempSync(join(tmpdir(), "cgstack-legacy-id-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      ...
      process.env.CGSTACK_HOSTNAME = "machine-a";
      const b = derivePathOnlyHashLegacyId(repo);
      process.env.CGSTACK_HOSTNAME = "machine-b";
      const c = derivePathOnlyHashLegacyId(repo);
      expect(a).toBe(b);
      expect(b).toBe(c);
      expect(a.startsWith("cgstack-code-")).toBe(true);
      expect(a.length).toBeLessThanOrEqual(32);
    } finally {
      delete process.env.CGSTACK_HOSTNAME;
      process.chdir(cwd);
      rmSync(repo, { recursive: true, force: true });
    }
  });
  ...
  it("produces a different id than the new hostname-folded form", () => {
    ...
    const repo = mkdtempSync(join(tmpdir(), "cgstack-legacy-id-distinct-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
    ...
    try {
      process.chdir(repo);
      process.env.CGSTACK_HOSTNAME = "machine-x";
      const legacy = derivePathOnlyHashLegacyId(repo);
      // Drive the new id through the CLI so we use the same code path users hit.
      const home = makeTestHome();
      const cgstackHome = join(home, ".cgstack");
      mkdirSync(cgstackHome, { recursive: true });
      const bindir = mkdtempSync(join(tmpdir(), "cgstack-legacy-id-distinct-bin-"));
      makeShim(bindir, { "--help": { stdout: "gbrain\n" } });
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        encoding: "utf-8",
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, CGSTACK_HOME: cgstackHome, CGSTACK_HOSTNAME: "machine-x", PATH: `${bindir}:${process.env.PATH || ""}` },
      });
...
describe("planHostnameFoldMigration", () => {
  ...
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "cgstack-mig-plan-bin-"));
    _resetGbrainSupportsRenameCache();
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  ...
  it("returns ids-match when legacy == new (degenerate case)", () => {
    const result = planHostnameFoldMigration("/repo/path", "cgstack-code-same-abc12345", "cgstack-code-same-abc12345");
...
describe("constrainSourceId truncation (hyphen-boundary cut)", () => {
  ...
  //
  // Exercised via the dry-run preview (`gbrain sources add cgstack-code-…`),
  // since constrainSourceId is module-private.
  it("never produces mid-word truncation artifacts like `kill` (from `skill`)", () => {
    const home = makeTestHome();
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-hyphen-cut-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
  ...
  it("produces a period-free source id for HTTPS remotes ending in .git (#1357)", () => {
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const repo = mkdtempSync(join(tmpdir(), "cgstack-https-period-"));
    spawnSync("git", ["init", "--quiet", "-b", "main"], { cwd: repo });
...
describe("sourceLocalPath", () => {
  let bindir: string;
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "cgstack-source-lp-bin-"));
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  });
