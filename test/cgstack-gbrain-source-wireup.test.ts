/**
 * cgstack-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the cgstack brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'cgstack-gbrain-source-wireup');

let tmpHome: string;
let cgstackHome: string;
...
function run(
  ...
  const env = {
    ...
    CGSTACK_BRAIN_WORKTREE: worktreeDir,
    CGSTACK_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupGstackRepo(remoteUrl: string) {
  // Real git repo at cgstackHome with at least one commit + an origin remote.
  fs.mkdirSync(cgstackHome, { recursive: true });
  spawnSync('git', ['-C', cgstackHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cgstackHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cgstackHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(cgstackHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', cgstackHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cgstackHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', cgstackHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-wireup-test-'));
  cgstackHome = path.join(tmpHome, '.cgstack');
  worktreeDir = path.join(tmpHome, '.cgstack-brain-worktree');
...
describe('cgstack-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupGstackRepo('git@github.com:user/cgstack-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('cgstack-brain-user');
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupGstackRepo('git@github.com:user/cgstack-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'cgstack-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove cgstack-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add cgstack-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('source-id derived from origin URL', () => {
    setupGstackRepo('git@github.com:user/cgstack-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('cgstack-brain-alice');
  });

  test('source-id fallback to ~/.cgstack-brain-remote.txt when .git is gone', () => {
    // No git repo at cgstackHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.cgstack-brain-remote.txt'),
      'git@github.com:user/cgstack-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .cgstack/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupGstackRepo('git@github.com:user/cgstack-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=cgstack-brain-user');
...
describe('cgstack-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('cgstack-gbrain-source-wireup — uninstall mode', () => {
...
describe('cgstack-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(cgstackHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', cgstackHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', cgstackHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', cgstackHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { CGSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
