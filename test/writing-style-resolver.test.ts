 ...
 * tier-≥2 preamble output, in multiple Codex passes host outputs. Since the
 ...
 * - Codex output uses $CGSTACK_BIN, not ~/.codex/... (host-aware paths)
...
function makeCtx(host: 'codex' | 'codex', tier: 1 | 2 | 3 | 4): TemplateContext {
  return {
...
describe('Writing Style preamble section', () => {
  test('tier 2+ Codex preamble includes Writing Style header', () => {
  ...
  test('Codex tier-2 preamble uses host-aware path (no .codex/)', () => {
    const out = generatePreamble(makeCtx('codex', 2));
    // The Writing Style section shouldn't reference a Codex-specific bin path.
    ...
    expect(explainLine).not.toMatch(/~\/\.codex\//);
    // Codex uses $CGSTACK_BIN
    expect(explainLine).toContain('$CGSTACK_BIN');
  });
