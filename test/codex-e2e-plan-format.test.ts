 ...
 *
 * Mirrors test/skill-e2e-plan-format.test.ts (the Codex version) but uses
 * test/helpers/codex-session-runner.ts to drive `codex exec` instead of `codex -p`.
 *
 * Four cases:
...
const CODEX_FORMAT_TOUCHFILES: Record<string, string[]> = {
  'codex-plan-ceo-format-mode':      ['.agents/skills/cgstack-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-ceo-format-approach':  ['.agents/skills/cgstack-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-coverage':  ['.agents/skills/cgstack-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-kind':      ['.agents/skills/cgstack-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
};
...
function setupCodexSkillDir(tmpPrefix: string, skillName: 'plan-ceo-review' | 'plan-eng-review'): { skillDir: string; planDir: string; outFile: string } {
  ...
  // Codex skill lives in .agents/skills/cgstack-{name}/ per the cgstack host convention.
  const codexSkillSource = path.join(ROOT, '.agents', 'skills', `cgstack-${skillName}`);
  const skillDir = path.join(planDir, '.agents', 'skills', `cgstack-${skillName}`);
...
// Capture instruction — same shape as the Codex version. Codex may ignore tool calls,
...
describeCodex('Codex Plan Format — CEO Mode Selection', () => {
  ...
  testIfSelected('codex-plan-ceo-format-mode', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'cgstack-plan-ceo-review',
...
describeCodex('Codex Plan Format — Eng Coverage Issue', () => {
  ...
  testIfSelected('codex-plan-eng-format-coverage', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'cgstack-plan-eng-review',
      sandbox: 'workspace-write',
    });
