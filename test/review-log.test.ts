...
function run(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/cgstack-review-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-revlog-'));
  // cgstack-review-log uses cgstack-slug which needs a git repo — create the projects dir
