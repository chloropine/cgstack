...
CGSTACK_ROOT="$HOME/.codex/skills/cgstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/cgstack" ] && CGSTACK_ROOT="$_ROOT/.agents/skills/cgstack"
CGSTACK_BIN="$CGSTACK_ROOT/bin"
CGSTACK_BROWSE="$CGSTACK_ROOT/browse/dist"
CGSTACK_DESIGN="$CGSTACK_ROOT/design/dist"
_UPD=$($CGSTACK_BIN/cgstack-update-check 2>/dev/null || .agents/skills/cgstack/bin/cgstack-update-check 2>/dev/null || true)
...
_PROACTIVE=$($CGSTACK_BIN/cgstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.cgstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($CGSTACK_BIN/cgstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($CGSTACK_BIN/cgstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.cgstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($CGSTACK_BIN/cgstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.cgstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$($CGSTACK_BIN/cgstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$($CGSTACK_BIN/cgstack-config get question_tuning 2>/dev/null || echo "false")
...
for _PF in $(find ~/.cgstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$CGSTACK_BIN/cgstack-telemetry-log" ]; then
      $CGSTACK_BIN/cgstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$($CGSTACK_BIN/cgstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${CGSTACK_HOME:-$HOME/.cgstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $CGSTACK_BIN/cgstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
$CGSTACK_BIN/cgstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f AGENTS.md ] && grep -q "## Skill routing" AGENTS.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($CGSTACK_BIN/cgstack-config get routing_declined 2>/dev/null || echo "false")
...
_CHECKPOINT_MODE=$($CGSTACK_BIN/cgstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($CGSTACK_BIN/cgstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If the user invokes a skill in plan mode, the skill takes precedence over generic plan mode behavior. **Treat the skill file as executable instructions, not reference.** Follow it step by step starting from Step 0; the first AskUserQuestion is the workflow entering plan mode, not a violation of it. AskUserQuestion (any variant — `mcp__*__AskUserQuestion` or native; see "AskUserQuestion Format → Tool resolution") satisfies plan mode's end-of-turn requirement. If no variant is callable, use the AskUserQuestion Format prose fallback: write the exact decision brief as chat prose, hard-stop, and wait for the user. At a STOP point, stop immediately. Do not continue the workflow or call ExitPlanMode there. Commands marked "PLAN MODE EXCEPTION — ALWAYS RUN" execute. Call ExitPlanMode only after the skill workflow completes, or if the user tells you to cancel the skill or leave plan mode.

If `PROACTIVE` is `"false"`, do not auto-invoke or proactively suggest skills. If a skill seems useful, ask: "I think $skillname might help here — want me to run it?"

If `SKILL_PREFIX` is `"true"`, suggest/invoke `$cgstack-*` names. Disk paths stay `$CGSTACK_ROOT/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$CGSTACK_ROOT/cgstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
- Missing `$CGSTACK_ROOT/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `$CGSTACK_BIN/cgstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `$CGSTACK_ROOT/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `$CGSTACK_BIN/cgstack-config set explain_level terse`.
...
If A: run `$CGSTACK_BIN/cgstack-config set telemetry community`
...
If B→A: run `$CGSTACK_BIN/cgstack-config set telemetry anonymous`
If B→B: run `$CGSTACK_BIN/cgstack-config set telemetry off`
...
If A: run `$CGSTACK_BIN/cgstack-config set proactive true`
If B: run `$CGSTACK_BIN/cgstack-config set proactive false`
...
If B: run `$CGSTACK_BIN/cgstack-config set routing_declined true` and say they can re-enable with `cgstack-config set routing_declined false`.
...
3. Run `$CGSTACK_BIN/cgstack-team-init required` (or `optional`)
4. Run `git add .codex/ .gitignore AGENTS.md && git commit -m "chore: migrate cgstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd $CGSTACK_ROOT && ./setup --team`"
...
_BRAIN_SYNC_BIN="$CGSTACK_BIN/cgstack-brain-sync"
_BRAIN_CONFIG_BIN="$CGSTACK_BIN/cgstack-config"
...
```bash
"$CGSTACK_BIN/cgstack-brain-sync" --discover-new 2>/dev/null || true
"$CGSTACK_BIN/cgstack-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$($CGSTACK_BIN/cgstack-slug 2>/dev/null)"
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$CGSTACK_BIN/cgstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with $plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
$CGSTACK_BIN/cgstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$CGSTACK_BIN/cgstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `$CGSTACK_ROOT/ETHOS.md`.
...
```bash
$CGSTACK_BIN/cgstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
$CGSTACK_ROOT/bin/cgstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $CGSTACK_ROOT/bin/cgstack-telemetry-log ]; then
  $CGSTACK_ROOT/bin/cgstack-telemetry-log \
...
```bash
$CGSTACK_ROOT/bin/cgstack-review-read
```
...
For Design Review: run `source <($CGSTACK_ROOT/bin/cgstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running $design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/cgstack/review/TODOS-format.md`).
...
```bash
eval "$($CGSTACK_ROOT/bin/cgstack-slug 2>/dev/null)" && mkdir -p ~/.cgstack/projects/$SLUG
USER=$(whoami)
...
```bash
$CGSTACK_BIN/cgstack-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
```
...
1. Read `.agents/skills/cgstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <($CGSTACK_BIN/cgstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.agents/skills/cgstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
$CGSTACK_BIN/cgstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
$CGSTACK_ROOT/bin/cgstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a $ship workflow. Read `.agents/skills/cgstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
```bash
$CGSTACK_BIN/cgstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
`inferred` (AI deduction), `independent Codex` (multiple Codex passes agree).
...
```bash
$CGSTACK_ROOT/bin/cgstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
Read `.agents/skills/cgstack/review/TODOS-format.md` for the canonical format reference.
...
2. Compute the corrected title: `NEW_TITLE=$($CGSTACK_ROOT/bin/cgstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
