 ...
 *
 * Costs ~$0.04 per run (4 Mini calls + 3 deterministic-only fixtures).
...
Stakes if we pick wrong: V1 ships months later, OR every other Codex session has to rebuild the same logic.
${recommendation}
Note: options differ in kind, not coverage — no completeness score.
Pros / cons:
A) Server-side (gbrain ships the smarts)
  ✅ Reusable across every Codex session that calls gbrain.
  ❌ Cross-repo work; gbrain release tied to cgstack release; slower V1
B) Client-side (cgstack ships the smarts) (recommended)
  ✅ Ships entirely in cgstack — no gbrain release dependency; faster V1
  ❌ Every other Codex session has to rebuild the same logic; multi-call overhead
...
describeIfSelected('judgeRecommendation rubric sanity', ['llm-judge-recommendation'], () => {
  testIfSelected('llm-judge-recommendation', async () => {
    ...
    const good5 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose C because hybrid ships V1 in cgstack-only without blocking on cross-repo gbrain coordination, and locks the migration path before other Codex sessions take a hard dependency.',
    ));
    ...
    const good4 = await judgeRecommendation(buildAUQ(
      'Recommendation: Choose B because client-side composition uses MCP tools that already exist in cgstack and avoids any gbrain release dependency for V1.',
    ));
...
Net: ...`);
    ...
    // INDEPENDENT CODEX synthesis recommendations: when Codex CLI or the Codex
    ...
    // fixtures pin the independent Codex review emit format documented in
    // bin/cgstack-codex-probe and scripts/resolvers/review.ts.
    // Substance-5 independent Codex fixtures explicitly compare against an
    ...
    // at least one alternative. Independent Codex synthesis has implicit
    // alternatives — different findings, different fix orders, ship-vs-fix —
    // so the same shape applies.
    const independentCodexCases = [
      [
        'codex-review good',
      ...
      // SUBSTANCE ~1-2: boilerplate independent Codex synthesis.
      [
        'independent Codex boilerplate',
        'Recommendation: Look at the findings because adversarial review found things.',
        false, // expect substance < 4
      ],
      [
        'independent Codex generic',
        'Recommendation: Ship as-is because the diff is fine.',
        false,
      ],
    ] as Array<[string, string, boolean]>;
    for (const [label, text, shouldPass] of independentCodexCases) {
      const score = await judgeRecommendation(text);
      expect(score.present, `[independent Codex:${label}] present should be true`).toBe(true);
      expect(score.has_because, `[independent Codex:${label}] has_because should be true`).toBe(true);
      if (shouldPass) {
        expect(
          score.reason_substance,
          `[independent Codex:${label}] expected substance >=4; got ${score.reason_substance}: ${score.reasoning}`,
        ).toBeGreaterThanOrEqual(4);
      } else {
        expect(
          score.reason_substance,
    ...
    // judge skips Mini when has_because is false). The "either B or C
    // because..." form does call Mini, but cost is bounded — total <$0.02.
    const hedgeForms = [
