...
const BIN = path.join(ROOT, 'bin', 'cgstack-paths');
...
// helper from inside a bash block (`eval "$(~/.codex/skills/cgstack/bin/cgstack-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`cgstack-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('cgstack-paths', () => {
  test('CGSTACK_HOME wins over CODEX_PLUGIN_DATA and HOME', () => {
    const got = run({
      CGSTACK_HOME: '/tmp/explicit-state',
      CODEX_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.CGSTACK_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CODEX_PLUGIN_DATA wins over HOME when CGSTACK_HOME unset', () => {
    const got = run({
      CODEX_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.CGSTACK_STATE_ROOT).toBe('/tmp/plugin-data');
  });

  test('HOME-derived state root when CGSTACK_HOME and CODEX_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.CGSTACK_STATE_ROOT).toBe('/tmp/myhome/.cgstack');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.CGSTACK_STATE_ROOT).toBe('.cgstack');
  });

  test('PLAN_ROOT chain: CGSTACK_PLAN_DIR > CODEX_PLANS_DIR > HOME > CWD', () => {
    expect(run({ CGSTACK_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
    expect(run({ CODEX_PLANS_DIR: '/tmp/codex', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/codex');
    expect(run({ HOME: '/tmp/myhome' }).PLAN_ROOT).toBe('/tmp/myhome/.codex/plans');
    // CWD fallback only verifiable on POSIX — Git Bash auto-populates HOME.
    if (process.platform !== 'win32') {
      expect(run({ HOME: '' }).PLAN_ROOT).toBe('.codex/plans');
    }
  });

  test('TMP_ROOT chain: TMPDIR > TMP > .cgstack/tmp', () => {
    expect(run({ TMPDIR: '/tmp/x', HOME: '/h' }).TMP_ROOT).toBe('/tmp/x');
    expect(run({ TMP: '/tmp/y', HOME: '/h' }).TMP_ROOT).toBe('/tmp/y');
    expect(run({ HOME: '' }).TMP_ROOT).toBe('.cgstack/tmp');
  });

  test('emits all three exports on every invocation', () => {
    const got = run({ HOME: '/tmp/h' });
    expect(got).toHaveProperty('CGSTACK_STATE_ROOT');
