/**
 * Unit tests for bin/cgstack-memory-ingest.ts (Lane A).
 *
 * Covers the unit-testable internals: parseTranscriptJsonl (Codex + Codex +
 ...
 *
 * E2E coverage (full --probe / --bulk on real ~/.codex/projects) lives in
...
const SCRIPT = join(import.meta.dir, "..", "bin", "cgstack-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "cgstack-memory-ingest-"));
}
...
function writeCodexCodeSession(home: string, projectName: string, sessionId: string, content: string): string {
  const projectsDir = join(home, ".codex", "projects", projectName);
...
describe("cgstack-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: cgstack-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds Codex sessions", () => {
    ...
    writeCodexCodeSession(home, "tmp-x", "abc123", session);
  ...
  it("--probe finds cgstack artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(join(cgstackHome, "analytics"), { recursive: true });
    mkdirSync(join(cgstackHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(cgstackHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(cgstackHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(cgstackHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(join(cgstackHome, "analytics"), { recursive: true });
    mkdirSync(join(cgstackHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(cgstackHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(cgstackHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("cgstack-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const cgstackHome = join(home, ".cgstack");
    mkdirSync(cgstackHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, CGSTACK_HOME: cgstackHome });
    expect(r.exitCode).toBe(0);
    const statePath = join(cgstackHome, ".transcript-ingest-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("cgstack-memory-ingest");
    rmSync(home, { recursive: true, force: true });
  });
...
describe("cgstack-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    // can be set by anyone who can write to ~/.codex/projects (cross-machine
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "cgstack-mi-cwd-marker-"));
    ...
    writeCodexCodeSession(home, "-tmp-target", "abc", record + "\n");

    const r = runScript(["--incremental", "--quiet"], {
      HOME: home,
      CGSTACK_HOME: cgstackHome,
      CGSTACK_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Codex JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "cgstack-mi-parse-"));
    ...
    const home = makeTestHome();
    const projDir = join(home, ".codex", "projects", "tmp-foo");
    mkdirSync(projDir, { recursive: true });
    writeFileSync(join(projDir, "abc123.jsonl"), content, "utf-8");

    const r = runScript(["--probe"], { HOME: home, CGSTACK_HOME: join(home, ".cgstack") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
  ...
  it("treats a truncated last line as partial (does not crash)", () => {
    const home = makeTestHome();
    const projDir = join(home, ".codex", "projects", "tmp-bar");
    mkdirSync(projDir, { recursive: true });
...
describe("cgstack-memory-ingest --limit", () => {
...
describe("cgstack-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
  ...
  it("invokes `gbrain import <dir> --no-embed --json` exactly once with hierarchical staging", () => {
    ...
    // Single Codex session fixture. --include-unattributed lets it
    // write even though there's no resolvable git remote in /tmp.
    const session =
      `{"type":"user","message":{"role":"user","content":"hi"},"timestamp":"2026-05-01T00:00:00Z","cwd":"/tmp/foo"}\n` +
      `{"type":"assistant","message":{"role":"assistant","content":"hello"},"timestamp":"2026-05-01T00:00:01Z"}\n`;
    writeCodexCodeSession(home, "tmp-foo", "abc123", session);
...
`;
    ...
    writeCodexCodeSession(home, "tmp-nul-test", "nul123", session);
...
`;
  ...
  it("D7: files listed in ~/.gbrain/sync-failures.jsonl are NOT recorded in state", () => {
    ...
    writeCodexCodeSession(home, "tmp-foo", "aaaa", sessionA);
    writeCodexCodeSession(home, "tmp-bar", "bbbb", sessionB);

    // Configure fake gbrain to "fail" the second session's staged path.
    // The staging-dir-relative path is "transcriptsCodex CLI/...bbbb.md"
...
`;
    ...
    const session =
      `{"type":"user","message":{"role":"user","content":"hi"},"timestamp":"2026-05-01T00:00:00Z","cwd":"/tmp/bar"}\n`;
    writeCodexCodeSession(home, "tmp-bar", "def456", session);
...
`;
    ...
    writeCodexCodeSession(home, "tmp-foo", "cleansess123", sessionA);
    // Force the path to contain the "dirty" marker.
    writeCodexCodeSession(home, "tmp-dirty-bar", "dirtysess456", sessionB);
