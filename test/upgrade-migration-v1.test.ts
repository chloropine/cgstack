/**
 * cgstack-upgrade/migrations/v1.0.0.0.sh — writing style migration.
 *
 * Coverage:
...
const MIGRATION = path.join(ROOT, 'cgstack-upgrade', 'migrations', 'v1.0.0.0.sh');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-mig-test-'));
});
