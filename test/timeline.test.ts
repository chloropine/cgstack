...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/cgstack-timeline-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runRead(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/cgstack-timeline-read ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-timeline-'));
...
describe('cgstack-timeline-read', () => {
