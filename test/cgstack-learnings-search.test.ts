...
const BIN = path.join(ROOT, 'bin', 'cgstack-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-search-cwd-'));
// cgstack-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('cgstack-learnings-search token-OR query semantics', () => {
