...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    cgstackHome = path.join(workDir, '.cgstack-home');
    ...
    // Compute slug (same logic as cgstack-slug without git remote)
  ...
  // Write a timeline event via cgstack-timeline-log, read it back via cgstack-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    ...
    const logBin = path.join(workDir, 'bin', 'cgstack-timeline-log');
    const readBin = path.join(workDir, 'bin', 'cgstack-timeline-read');
    const env = { ...process.env, CGSTACK_HOME: cgstackHome };
    ...
    // Read via cgstack-timeline-read
    ...
    // Verify cgstack-timeline-read output includes the events
...
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.codex/skills/cgstack/bin/.
  Replace any references to ~/.codex/skills/cgstack/bin/ with ./bin/ when running commands.
