/**
 * Regression: no stale `cgstack-brain-init`, `gbrain_sync_mode`, or
 * `~/.cgstack-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`cgstack-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'cgstack-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'cgstack-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/cgstack-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/cgstack-config comment explicitly preserves the rename note.
  'bin/cgstack-config',
  // Detect script's "renamed in v1.27.0.0" comment + brain-remote-fallback path.
  'bin/cgstack-gbrain-detect',
  ...
  'bin/cgstack-brain-restore',
  'bin/cgstack-gbrain-source-wireup',
  'bin/cgstack-brain-uninstall',
  ...
  // cgstack-upgrade.test.ts may exercise old migration behavior.
  'test/cgstack-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-cgstack-brain-refs.test.ts',
...
describe('no stale cgstack-brain refs (v1.27.0.0 rename)', () => {
