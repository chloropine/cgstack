...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const cgstackHome = path.join(opDir, '.cgstack-home');
    ...
    for (const script of ['cgstack-learnings-log', 'cgstack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // cgstack-learnings-log will create the project dir automatically via cgstack-slug
...
\`\`\`bash
CGSTACK_HOME="${cgstackHome}" ${binDir}/cgstack-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(cgstackHome, 'projects');
