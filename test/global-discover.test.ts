...
const scriptPath = join(import.meta.dir, "..", "bin", "cgstack-global-discover.ts");

describe("cgstack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/cgstack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:chloropine/cgstack.git");
      const https = normalizeRemoteUrl("https://github.com/chloropine/cgstack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/chloropine/cgstack");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "cgstack-codex-test-"));
  ...
  describe("discovery output structure", () => {
    ...
    test("tools summary matches repo data", () => {
      ...
      const toolTotal = json.tools.codex.total_sessions;
