/**
 * cgstack-artifacts-url — URL canonicalization helper.
 *
...
const URL_BIN = path.join(ROOT, 'bin', 'cgstack-artifacts-url');
...
describe('cgstack-artifacts-url', () => {
  test('--to ssh from canonical https', () => {
    const r = run(['--to', 'ssh', 'https://github.com/chloropine/cgstack-artifacts']);
    expect(r.code).toBe(0);
    expect(r.stdout).toBe('git@github.com:chloropine/cgstack-artifacts.git');
  });

  test('--to ssh from https-with-.git', () => {
    const r = run(['--to', 'ssh', 'https://github.com/chloropine/cgstack-artifacts.git']);
    expect(r.stdout).toBe('git@github.com:chloropine/cgstack-artifacts.git');
  });

  test('--to https is idempotent on https input', () => {
    const r = run(['--to', 'https', 'https://github.com/chloropine/cgstack-artifacts']);
    expect(r.stdout).toBe('https://github.com/chloropine/cgstack-artifacts');
  });

  test('--to https from git@host:owner/repo.git', () => {
    const r = run(['--to', 'https', 'git@github.com:chloropine/cgstack-artifacts.git']);
    expect(r.stdout).toBe('https://github.com/chloropine/cgstack-artifacts');
  });

  test('--to https from ssh:// scheme (gitlab self-hosted style)', () => {
    const r = run(['--to', 'https', 'ssh://git@gitlab.example.org/team/cgstack-artifacts-team.git']);
    expect(r.stdout).toBe('https://gitlab.example.org/team/cgstack-artifacts-team');
  });
  ...
  test('--owner-repo extracts the path segment', () => {
    expect(run(['--owner-repo', 'https://github.com/chloropine/cgstack-artifacts']).stdout)
      .toBe('chloropine/cgstack-artifacts');
    expect(run(['--owner-repo', 'git@github.com:team/cgstack-artifacts-team.git']).stdout)
      .toBe('team/cgstack-artifacts-team');
  });
  ...
  test('round-trip: https → ssh → https is identity', () => {
    const original = 'https://github.com/chloropine/cgstack-artifacts';
