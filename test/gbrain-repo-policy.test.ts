/**
 * cgstack-gbrain-repo-policy — per-remote trust-tier policy store.
 *
 ...
 *
 * Each test uses a temp CGSTACK_HOME so nothing leaks into the user's real ~/.cgstack.
 */
...
const BIN = path.join(ROOT, 'bin', 'cgstack-gbrain-repo-policy');
