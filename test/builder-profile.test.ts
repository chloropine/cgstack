...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/cgstack-builder-profile`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cgstack-profile-'));
});
...
describe('cgstack-builder-profile', () => {
