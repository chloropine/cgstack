// Unit tests for cgstack-upgrade/migrations/v1.38.1.0.sh (#1452).
...
const MIGRATION = join(REPO_ROOT, 'cgstack-upgrade', 'migrations', 'v1.38.1.0.sh');

function setupFakeHome(): string {
  const dir = mkdtempSync(join(tmpdir(), 'mig-v1340-'));
  mkdirSync(join(dir, '.cgstack'), { recursive: true });
  return dir;
}
...
describe('v1.38.1.0 migration', () => {
  test('adds patterns to allowlist before USER ADDITIONS marker', () => {
    ...
    try {
      writeFileSync(join(home, '.cgstack', '.brain-allowlist'), [
      ...
      const content = readFileSync(join(home, '.cgstack', '.brain-allowlist'), 'utf-8');
  ...
  test('adds entries to privacy-map.json via jq (preserves JSON validity)', () => {
    const home = setupFakeHome();
    try {
      writeFileSync(join(home, '.cgstack', '.brain-privacy-map.json'), JSON.stringify([
      ...
      const raw = readFileSync(join(home, '.cgstack', '.brain-privacy-map.json'), 'utf-8');
  ...
  test('adds union-merge rules to gitattributes', () => {
    const home = setupFakeHome();
    try {
      writeFileSync(join(home, '.cgstack', '.gitattributes'), [
      ...
      const content = readFileSync(join(home, '.cgstack', '.gitattributes'), 'utf-8');
  ...
  test('is idempotent: re-running on already-patched files is a no-op', () => {
    ...
    try {
      ...
      writeFileSync(join(home, '.cgstack', '.gitattributes'), '*.jsonl merge=jsonl-append\n');

      runMigration(home);
      // Remove the done marker so re-run actually executes
      rmSync(join(home, '.cgstack', '.migrations'), { recursive: true, force: true });

      const beforeAllowlist = readFileSync(join(home, '.cgstack', '.brain-allowlist'), 'utf-8');
      const beforePrivacy = readFileSync(join(home, '.cgstack', '.brain-privacy-map.json'), 'utf-8');
      const beforeAttrs = readFileSync(join(home, '.cgstack', '.gitattributes'), 'utf-8');

      runMigration(home);

      const afterAllowlist = readFileSync(join(home, '.cgstack', '.brain-allowlist'), 'utf-8');
      const afterPrivacy = readFileSync(join(home, '.cgstack', '.brain-privacy-map.json'), 'utf-8');
      const afterAttrs = readFileSync(join(home, '.cgstack', '.gitattributes'), 'utf-8');
  ...
  test('repairs privacy-map even when allowlist is missing (per-file independence)', () => {
    ...
    try {
      ...
      const parsed = JSON.parse(readFileSync(join(home, '.cgstack', '.brain-privacy-map.json'), 'utf-8'));
      const patterns = parsed.map((e: any) => e.pattern);
      expect(patterns).toContain('projects/*/*-design-*.md');
      // Allowlist remains absent (we don't create files that weren't there)
      expect(existsSync(join(home, '.cgstack', '.brain-allowlist'))).toBe(false);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  });
  ...
  test('migration marker prevents re-running', () => {
    const home = setupFakeHome();
    try {
      writeFileSync(join(home, '.cgstack', '.brain-allowlist'), '# ---- USER ADDITIONS BELOW\n');
      runMigration(home);
      // Confirm marker file exists
      expect(existsSync(join(home, '.cgstack', '.migrations', 'v1.38.1.0.done'))).toBe(true);

      // Modify allowlist so we can detect if the migration would re-run
      writeFileSync(join(home, '.cgstack', '.brain-allowlist'), '# minimal\n');

      runMigration(home);

      // With the marker present, the migration short-circuits, so the file
      // we just wrote stays unmodified
      expect(readFileSync(join(home, '.cgstack', '.brain-allowlist'), 'utf-8')).toBe('# minimal\n');
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  });
...
// ──────────────────────────────────────────────────────────────────────────
const MIGRATION_V1_40 = join(REPO_ROOT, 'cgstack-upgrade', 'migrations', 'v1.40.0.0.sh');
...
describe('v1.40.0.0 migration', () => {
  test('adds eng-review-test-plan pattern to allowlist on top of an installed v1.38.1.0 state', () => {
    ...
    try {
      ...
      mkdirSync(join(home, '.cgstack', '.migrations'), { recursive: true });
      writeFileSync(join(home, '.cgstack', '.migrations', 'v1.38.1.0.done'), '');
      writeFileSync(join(home, '.cgstack', '.brain-allowlist'), [
        'projects/*/learnings.jsonl',
        'projects/*/designs/*.md',
        'projects/*/*-design-*.md',
      ...
      expect(existsSync(join(home, '.cgstack', '.migrations', 'v1.40.0.0.done'))).toBe(true);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
  });
