...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'cgstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="cgstack-pill-dot"></span> cgstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'cgstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'cgstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'cgstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'cgstack-ref-panel-header';
  header.textContent = `cgstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'cgstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'cgstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'cgstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'cgstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'cgstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'cgstack-ref-panel-more';
...
// Do NOT dispatch cgstack-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('cgstack-extension-ready'));
    return;
  }
