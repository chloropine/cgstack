#!/usr/bin/env bun
/**
 * Generate cgstack/llms.txt — a single discoverable index of every cgstack
 ...
 *
 * Output: cgstack/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, 'cgstack', 'llms.txt');
 ...
 * the cgstack convention; we collapse those into a single paragraph.
 */
 ...
 * in a full cgstack checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/chloropine/cgstack');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate cgstack/llms.txt');
        process.exit(1);
      }
