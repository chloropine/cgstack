#!/bin/bash
# Build CGStack Browser.app — macOS application bundle
#
...
#
# Output: dist/CGStack Browser.app and dist/CGStack-Browser.dmg
#
# Usage:
...
APP_NAME="CGStack Browser"
BUNDLE_ID="com.cgstack.browser"
...
# Launcher script
cp "$ROOT/scripts/app/cgstack-browser" "$APP_DIR/Contents/MacOS/cgstack-browser"
chmod +x "$APP_DIR/Contents/MacOS/cgstack-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "CGStack Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the CGStack icon
...
echo "    Contents/MacOS/cgstack-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/CGStack-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"
