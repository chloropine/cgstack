#!/usr/bin/env bun
/**
 * analytics — CLI for viewing cgstack skill usage statistics.
 *
 * Reads ~/.cgstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.cgstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('cgstack skill usage analytics');
