 ...
 *
 * Learnings are stored per-project at ~/.cgstack/projects/{slug}/learnings.jsonl.
 ...
 * by cgstack-learnings-search ("latest winner" per key+type).
 *
 * Cross-project discovery is opt-in. The resolver asks the user once via
 * AskUserQuestion and persists the preference via cgstack-config.
 */
...
// queries hand-written in cgstack are safe, but the resolver API must defend against
...
export function generateLearningsSearch(ctx: TemplateContext, args?: string[]): string {
  ...
  if (ctx.host === 'codex') {
    // Codex: simpler version, no cross-project, uses $CGSTACK_BIN
    return `## Prior Learnings

Search for relevant learnings from previous sessions on this project:

\`\`\`bash
$CGSTACK_BIN/cgstack-learnings-search --limit 10${queryFlag} 2>/dev/null || true
\`\`\`
...
\`\`\`bash
_CROSS_PROJ=$(${ctx.paths.binDir}/cgstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${ctx.paths.binDir}/cgstack-learnings-search --limit 10${queryFlag} --cross-project 2>/dev/null || true
else
  ${ctx.paths.binDir}/cgstack-learnings-search --limit 10${queryFlag} 2>/dev/null || true
fi
\`\`\`
...
If A: run \`${ctx.paths.binDir}/cgstack-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/cgstack-config set cross_project_learnings false\`
...
export function generateLearningsLog(ctx: TemplateContext): string {
  const binDir = ctx.host === 'codex' ? '$CGSTACK_BIN' : ctx.paths.binDir;
...
\`\`\`bash
${binDir}/cgstack-learnings-log '{"skill":"${ctx.skillName}","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
\`\`\`
