 ...
 *      during the v1.27.0.0 migration window) exists AND ~/.cgstack/.git is
 *      missing, surface a restore-available hint (does NOT auto-run restore).
 *   2. If sync is on, run `cgstack-brain-sync --once` (drain + push). The
 ...
 * to `cgstack-brain-sync --discover-new` + `--once`.
 */
import type { TemplateContext } from '../types';

export function generateBrainSyncBlock(ctx: TemplateContext): string {
  const isBrainHost = ctx.host === 'gbrain' || ctx.host === 'codex';
...
${isBrainHost ? `If output shows \`ARTIFACTS_SYNC: artifacts repo detected\`, offer \`cgstack-brain-restore\` via AskUserQuestion; otherwise continue.` : ''}
...
\`\`\`bash
"${ctx.paths.binDir}/cgstack-brain-sync" --discover-new 2>/dev/null || true
"${ctx.paths.binDir}/cgstack-brain-sync" --once 2>/dev/null || true
\`\`\`
`;
}
