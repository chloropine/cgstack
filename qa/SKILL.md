...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use $qa-only. (cgstack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
| Mode | full | `--regression .cgstack/qa-reports/baseline.json` |
| Output dir | `.cgstack/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .cgstack/qa-reports/screenshots
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.codex/skills/cgstack/bin/cgstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  ~/.codex/skills/cgstack/bin/cgstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
1. **Project-scoped test plans:** Check `~/.cgstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.codex/skills/cgstack/bin/cgstack-slug 2>/dev/null)"
   ls -t ~/.cgstack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
- Include full attribution comment:
  ...
  // Report: .cgstack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.cgstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.codex/skills/cgstack/bin/cgstack-slug 2>/dev/null)" && mkdir -p ~/.cgstack/projects/$SLUG
```
Write to `~/.cgstack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
~/.codex/skills/cgstack/bin/cgstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
