 ...
 * Marks hidden elements with data-cgstack-hidden attribute.
  ...
  return page.evaluate((ariaPatterns: string[]) => {
    ...
    for (const el of elements) {
      if (el instanceof HTMLElement) {
        ...
        if (isHidden) {
          el.setAttribute('data-cgstack-hidden', 'true');
...
export async function getCleanTextWithStripping(page: Page | Frame): Promise<string> {
  const raw = await page.evaluate(() => {
    ...
    clone.querySelectorAll('[data-cgstack-hidden]').forEach(el => el.remove());
...
/**
 * Clean up data-cgstack-hidden attributes from the page.
 * Should be called after extraction is complete.
 */
export async function cleanupHiddenMarkers(page: Page | Frame): Promise<void> {
  await page.evaluate(() => {
    document.querySelectorAll('[data-cgstack-hidden]').forEach(el => {
      el.removeAttribute('data-cgstack-hidden');
    });
  });
}
