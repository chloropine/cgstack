...
function formatSavedOk(row: DomainSkillRow, slug: string): string {
  return [
    ...
    `Version: ${row.version}`,
    `Stored at: ~/.cgstack/projects/${slug}/learnings.jsonl`,
    '',
...
async function handleEdit(args: string[]): Promise<string> {
  ...
  const tmpFile = path.join(os.tmpdir(), `cgstack-domain-skill-${process.pid}-${Date.now()}.md`);
...
async function handlePromoteToGlobal(args: string[]): Promise<string> {
  ...
  return [
    ...
    `Stored at: ~/.cgstack/global-domain-skills.jsonl`,
    '',
