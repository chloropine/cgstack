/**
 * Tests for bin/cgstack-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CGSTACK_DIR / CGSTACK_STATE_DIR / CGSTACK_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cgstack-update-check');
...
beforeEach(() => {
  cgstackDir = mkdtempSync(join(tmpdir(), 'cgstack-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'cgstack-state-test-'));
  // Link real cgstack-config so update_check config check works
  const binDir = join(cgstackDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'cgstack-config'), join(binDir, 'cgstack-config'));
});

afterEach(() => {
  rmSync(cgstackDir, { recursive: true, force: true });
...
describe('cgstack-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.4.0\n');
  ...
  test('just-upgraded marker does not mask newer remote version', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('just-upgraded with no further updates writes UP_TO_DATE cache', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.4.0\n');
    writeFileSync(join(stateDir, 'just-upgraded-from'), '0.3.3\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.3.3\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      CGSTACK_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ CGSTACK_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(cgstackDir, 'VERSION'), version + '\n');
  ...
  test('remote older than local (stale CDN) → silent, cache UP_TO_DATE', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '1.34.0.0\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '1.33.2.0\n');
  ...
  test('multi-segment sort: 1.9.0.0 < 1.10.0.0', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '1.9.0.0\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '1.10.0.0\n');
  ...
  test('multi-segment reverse sort: 1.10.0.0 > 1.9.0.0 → no rewind', () => {
    writeFileSync(join(cgstackDir, 'VERSION'), '1.10.0.0\n');
    writeFileSync(join(cgstackDir, 'REMOTE_VERSION'), '1.9.0.0\n');
