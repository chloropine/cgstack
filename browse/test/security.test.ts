...
describe('canary', () => {
  ...
  test('injected canary is detected when echoed', () => {
    ...
    // Attacker crafts Codex output that echoes the canary
...
describe('logAttempt', () => {
  test('writes attempts.jsonl with correct shape', () => {
    ...
    const logPath = path.join(os.homedir(), '.cgstack', 'security', 'attempts.jsonl');
...
describe('resolveBashBinary', () => {
  ...
  test('honors CGSTACK_BASH_BIN absolute-path override', () => {
    ...
    const out = resolveBashBinary({ CGSTACK_BASH_BIN: fake, PATH: '' });
  ...
  test('BASH_BIN works as a fallback when CGSTACK_BASH_BIN is unset', () => {
    const fake = process.platform === 'win32' ? 'C:\\opt\\bash.exe' : '/opt/custom/bash';
...
describe('buildTelemetrySpawnCommand', () => {
  const bin = '/home/user/.agents/skills/cgstack/bin/cgstack-telemetry-log';
  ...
  test('on win32 with bash resolvable, wraps the call in bash with the script as first arg', () => {
    ...
    const out = buildTelemetrySpawnCommand(bin, args, { CGSTACK_BASH_BIN: fakeBash, PATH: '' });
    expect(out).not.toBeNull();
