...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/cgstack-cli-');
    const execPath = path.join(root, '.agents/skills/cgstack/browse/dist/browse');
    const serverPath = path.join(root, '.agents/skills/cgstack/browse/src/server.ts');
