/**
 * Tests for bin/cgstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * CGSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'cgstack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'cgstack-config-test-'));
});
...
describe('cgstack-config', () => {
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# cgstack configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# cgstack configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# cgstack configuration');
  });
