 ...
 *
 *   stageSkill    → writes files into ~/.cgstack/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.cgstack/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.cgstack', '.tmp');
});
