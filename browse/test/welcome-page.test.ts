...
describe('welcome page served via HTTP', () => {
  ...
  test('has CGStack Browser title and branding', () => {
    expect(html).toContain('<title>CGStack Browser</title>');
    expect(html).toContain('CGStack Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'cgstack-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('cgstack-status-pill');
  ...
  test('has footer with attribution', () => {
    ...
    expect(html).toContain('github.com/chloropine/cgstack');
  });
});
