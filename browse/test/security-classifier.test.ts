 ...
 *
 * Scope: functions that do NOT require model download, codex CLI, or
 * network access. Model-dependent behavior (loadTestsavant inference,
 * checkTranscript Mini calls) belongs in a smoke harness that pulls
...
describe('shouldRunTranscriptCheck — Mini gating optimization', () => {
  test('returns false when no layer has fired at >= LOG_ONLY', () => {
    // Clean pre-tool-call: no classifier saw anything interesting.
    // Skipping Mini here is the 70% savings described in plan §E1.
  ...
  test('returns true when testsavant_content fires at LOG_ONLY threshold', () => {
    // Exactly at 0.40 — should trigger Mini follow-up.
    const signals: LayerSignal[] = [
  ...
  test('returns true when aria_regex alone fires above LOG_ONLY', () => {
    // Regex hit on its own is suspicious enough to warrant Mini second opinion.
    const signals: LayerSignal[] = [
  ...
  test('does NOT gate on transcript_classifier itself (no recursion)', () => {
    ...
    // the new tool call shouldn't re-trigger Mini based on the previous
    // transcript signal alone — we need a fresh content signal. This
    // prevents feedback loops where one Mini hit forever gates future calls.
    const signals: LayerSignal[] = [
  ...
  test('empty signals list returns false (no reason to call Mini)', () => {
...
describe('getClassifierStatus — pre-load state', () => {
  test('returns testsavant=off before loadTestsavant has been called', () => {
    ...
    // transcript starts 'off' until first checkMiniAvailable() call
