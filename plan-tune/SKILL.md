...
version: 1.0.0
description: |
  Self-tuning question sensitivity + developer psychographic for cgstack (v1: observational).
  Review which AskUserQuestion prompts fire across cgstack skills, set per-question preferences
  ...
  "developer profile", or "turn off question tuning". (cgstack)

  Proactively suggest when the user says the same cgstack question has come up before,
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
7. **"Turn it off" / "disable"** → `~/.codex/skills/cgstack/bin/cgstack-config set question_tuning false`
8. **"Turn it on" / "enable"** → `~/.codex/skills/cgstack/bin/cgstack-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$(~/.codex/skills/cgstack/bin/cgstack-config get question_tuning 2>/dev/null || echo "false")
   echo "QUESTION_TUNING: $_QT"
   ```

2. If `false`, use AskUserQuestion:

   > Question tuning is off. cgstack can learn which of its prompts you find
   > valuable vs noisy — so over time, cgstack stops asking questions you've
   > already answered the same way. It takes about 2 minutes to set up your
   > initial profile. v1 is observational: cgstack tracks your preferences
...
3. If A or B: enable:
   ```bash
   ~/.codex/skills/cgstack/bin/cgstack-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `~/.cgstack/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   ~/.codex/skills/cgstack/bin/cgstack-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$(~/.codex/skills/cgstack/bin/cgstack-paths)"
   _PROFILE="$CGSTACK_STATE_ROOT/developer-profile.json"
   bun -e "
     const fs = require('fs');
...
```bash
~/.codex/skills/cgstack/bin/cgstack-developer-profile --profile
```

Parse the JSON. Present in **plain English**, not raw floats:

- For each dimension where `declared[dim]` is set, transform to a plain-English
...
- Show the vibe (archetype) from `cgstack-developer-profile --vibe` — the
...
eval "$(~/.codex/skills/cgstack/bin/cgstack-slug 2>/dev/null)"
eval "$(~/.codex/skills/cgstack/bin/cgstack-paths)"
_LOG="$CGSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
If `NO_LOG`, tell the user: "No questions logged yet. As you use cgstack skills,
cgstack will log them here."
...
4. Write:
   ```bash
   ~/.codex/skills/cgstack/bin/cgstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.codex/skills/cgstack/bin/cgstack-developer-profile --gap
```
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-preference --stats
eval "$(~/.codex/skills/cgstack/bin/cgstack-slug 2>/dev/null)"
eval "$(~/.codex/skills/cgstack/bin/cgstack-paths)"
_LOG="$CGSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
~/.codex/skills/cgstack/bin/cgstack-developer-profile --profile | bun -e "
