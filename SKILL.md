...
description: |
  ...
  test a site, verify a deployment, dogfood a user flow, or file a bug with screenshots. (cgstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"cgstack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"cgstack","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
The user has context you do not. Independent Codex agreement is a recommendation, not a decision. The user decides.
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other cgstack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `cgstack-config`.
...
- User asks for a second opinion, codex review → use Codex CLI review
- User asks for safety mode, careful mode → invoke `$careful` or `$guard`
- User asks to restrict edits to a directory → invoke `$freeze` or `$unfreeze`
- User asks to upgrade cgstack → invoke `$cgstack-upgrade`
...
- User asks to launch a real browser for QA, "open the browser" → invoke `$open-cgstack-browser`
...
- User asks what cgstack has learned, "show learnings" → invoke `$learn`
...
If the user opts out of suggestions, run `cgstack-config set proactive false`.
If they opt back in, run `cgstack-config set proactive true`.

# cgstack browse: QA Testing & Dogfooding
...
- NEVER use `mcp__codex-in-chrome__*` tools. They are slow and unreliable.
...
`$B` is the browse binary (resolved from `$_ROOT/.agents/skills/cgstack/browse/dist/browse` or `~/.codex/skills/cgstack/browse/dist/browse`).
