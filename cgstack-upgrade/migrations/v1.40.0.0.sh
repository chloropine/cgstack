...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "  [v1.40.0.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run cgstack-upgrade, or run cgstack-artifacts-init manually." >&2
  fi
fi
...
# bin/cgstack-artifacts-init now writes the pattern directly, so re-runs
