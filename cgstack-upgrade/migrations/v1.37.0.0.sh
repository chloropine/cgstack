...
#       ~/.codex/skills/cgstack/bin/cgstack-config set local_code_index_offered true
#
# When silent: anything else (Path 1/2/3 users, anyone already on PGLite,
# anyone who opted out, anyone without remote-http MCP).
#
# Idempotency: writes a touchfile at ~/.cgstack/.migrations/v1.37.0.0.done
...
MIGRATIONS_DIR="$CGSTACK_HOME/.migrations"
DONE_TOUCH="$MIGRATIONS_DIR/v1.37.0.0.done"
CONFIG_BIN="$HOME/.codex/skills/cgstack/bin/cgstack-config"
...
is_remote_http_mcp() {
  [ -f "$CODEX_JSON" ] || return 1
  command -v jq >/dev/null 2>&1 || return 1
  local mtype murl
  mtype=$(jq -r '.mcpServers.gbrain.type // .mcpServers.gbrain.transport // empty' "$CODEX_JSON" 2>/dev/null)
  murl=$(jq -r '.mcpServers.gbrain.url // empty' "$CODEX_JSON" 2>/dev/null)
  case "$mtype" in
...
if is_remote_http_mcp && is_local_engine_missing; then
  ...
  ┌──────────────────────────────────────────────────────────────────┐
  │  cgstack v1.37.0.0 — split-engine gbrain                          │
  │                                                                  │
