#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename cgstack-brain-* → cgstack-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename cgstack-brain-$USER →
#                              cgstack-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.cgstack-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.cgstack/config.yaml
#   4. codex_md_block_rewritten — find-and-replace any existing GBrain
...
#
# Interruption-safe via journal at ~/.cgstack/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.cgstack/.migrations/v1.27.0.0.done.
#
...
# Configuration
# ---------------------------------------------------------------------------
CGSTACK_HOME="${HOME}/.cgstack"
SKILLS_DIR="${HOME}/.codex/skills"
BIN_DIR="${SKILLS_DIR}/cgstack/bin"
CONFIG_BIN="${BIN_DIR}/cgstack-config"
URL_BIN="${BIN_DIR}/cgstack-artifacts-url"

MIGRATION_DIR="${CGSTACK_HOME}/.migrations"
...
OLD_REPO_NAME="cgstack-brain-${USER_NAME}"
NEW_REPO_NAME="cgstack-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.cgstack-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.cgstack-artifacts-remote.txt"
...
#   - presence of ~/.cgstack-brain-remote.txt (legacy file)
#   - presence of ~/.cgstack/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$CGSTACK_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy cgstack-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  This migration will:
    ...
    2. mv ~/.cgstack-brain-remote.txt → ~/.cgstack-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.cgstack/config.yaml
    4. Update any "## GBrain Configuration" block in AGENTS.md
...
# ---------------------------------------------------------------------------
detect_mcp_mode() {
  # Cheap probe: ~/.codex.json type field. Defense-in-depth tier 3 only;
  # the migration script avoids invoking `codex` to keep upgrade fast.
  if command -v jq >/dev/null 2>&1 && [ -f "$HOME/.codex.json" ]; then
    local t
    t=$(jq -r '.mcpServers.gbrain.type // .mcpServers.gbrain.transport // empty' "$HOME/.codex.json" 2>/dev/null)
    case "$t" in
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.cgstack-brain-remote.txt → ~/.cgstack-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.cgstack-brain-remote.txt → ~/.cgstack-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # cgstack-brain-$USER with cgstack-artifacts-$USER in the URL.
...
# ---------------------------------------------------------------------------
# Step 4: rewrite AGENTS.md "## GBrain Configuration" block fields
# ---------------------------------------------------------------------------
if ! journal_done "codex_md_block_rewritten"; then
  ...
  mark_done "codex_md_block_rewritten"
fi
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$CGSTACK_HOME/.git" ]; then
    ...
    WORKTREE="${CGSTACK_BRAIN_WORKTREE:-$HOME/.cgstack-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.cgstack/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi
