...
#
# Why a migration: cgstack-artifacts-init regenerates these files but also
# does `git commit + push` on ~/.cgstack/, which would clobber user state on
...
ALLOWLIST="${CGSTACK_HOME}/.brain-allowlist"
PRIVACY="${CGSTACK_HOME}/.brain-privacy-map.json"
GITATTRS="${CGSTACK_HOME}/.gitattributes"
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "  [v1.38.1.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run cgstack-upgrade, or run cgstack-artifacts-init manually." >&2
  fi
fi
...
# patches ship into their federated artifacts repo (next cgstack-brain-sync
