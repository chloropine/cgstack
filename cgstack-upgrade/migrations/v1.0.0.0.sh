...
# the older V0 prose can set `cgstack-config set explain_level terse`.
#
...
PROMPTED_FLAG="$CGSTACK_HOME/.writing-style-prompted"
PENDING_FLAG="$CGSTACK_HOME/.writing-style-prompt-pending"

mkdir -p "$CGSTACK_HOME"
...
EXPLAIN_LEVEL_SET="$("${HOME}/.agents/skills/cgstack/bin/cgstack-config" get explain_level 2>/dev/null || true)"
