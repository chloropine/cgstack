...
#   - No ~/.cgstack/.git (brain-init never ran)
...
WIREUP_BIN="${BIN_DIR}/cgstack-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: cgstack-config can emit trailing newlines,
