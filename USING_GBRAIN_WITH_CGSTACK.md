...
**Trust warning:** Pasting this URL gives your local Codex full read/write access to every page in the shared brain. If that's not the trust level you want, pick PGLite local (Path 3) instead and accept the brains are disjoint.
...
**What happens:** You paste an MCP URL (e.g. `https://wintermute.tail554574.ts.net:3131/mcp`) and a bearer token. The skill verifies the URL over the wire, registers gbrain as an HTTP MCP in `~/.codex.json` at user scope, and offers to also stand up a tiny local PGLite for code search (~30 seconds, ~120 MB disk).
...
By default the skill asks "Give Codex a typed tool surface for gbrain?" If you say yes, it runs:

```bash
codex mcp add gbrain -- gbrain serve
```

That registers gbrain's stdio MCP server with Codex. Now `gbrain search`, `gbrain put_page`, `gbrain get_page`, etc. show up as first-class tools in every session, not bash shell-outs.

**If `codex` is not on PATH**, the skill skips MCP registration gracefully with a manual-register hint. The CLI resolver still works from any skill that shells out to `gbrain` — MCP is an upgrade, not a prerequisite.

cgstack registers gbrain only for Codex. Other host MCP registration is outside this fork's scope.
...
The skill asks once per repo the first time you run a cgstack skill there. After that the decision is sticky — every worktree + branch of the same git remote shares the same policy, so you set it once and it follows you.
...
```bash
~/.codex/skills/cgstack/bin/cgstack-gbrain-repo-policy list
```
...
3. **Memory stage.** Stages your `~/.cgstack/` transcripts + curated memory. In local-stdio MCP mode, ingests into the local engine. In remote-http MCP mode, persists staged markdown to `~/.cgstack/transcripts/run-<pid>-<ts>/` for the remote brain admin's pull pipeline.
...
5. **AGENTS.md guidance.** Capability-checks the round-trip (write a page → search → find it). If green, writes the `## GBrain Search Guidance` block to your project's AGENTS.md. If red, REMOVES the block — the agent should never be told to use a tool that isn't installed.
...
Re-runnable, idempotent, safe to run from multiple terminals on the same machine (locked at `~/.cgstack/.sync-gbrain.lock`).
...
**If migration hangs:** another cgstack session may be holding a lock on the source brain. The timeout fires at 3 minutes with an actionable message. Close other workspaces and re-run.

## CGStack memory sync (a separate concern)

This is different from gbrain itself. Your cgstack state (`~/.cgstack/` — learnings, plans, retros, timeline, developer profile) is machine-local by default. "CGStack memory sync" optionally pushes a curated, secret-scanned subset to a private git repo so your memory follows you across machines — and, if you're running gbrain, that git repo becomes indexable there too.
...
**On a new machine:** Copy `~/.cgstack-brain-remote.txt` over, run `cgstack-brain-restore`, and yesterday's learnings surface on today's laptop.
...
| Bin | Purpose |
|---|---|
| `cgstack-gbrain-detect` | Emit current state as JSON: gbrain on PATH, version, config engine, doctor status, sync mode |
| `cgstack-gbrain-install` | Detect-first installer (probes `~/git/gbrain`, `~/gbrain`, then fresh clone). Has `--dry-run` and `--validate-only` flags. PATH-shadow check exits 3 with remediation menu. |
| `cgstack-gbrain-lib.sh` | Sourced, not executed. Provides `read_secret_to_env VARNAME "prompt" [--echo-redacted "<sed-expr>"]` |
| `cgstack-gbrain-supabase-verify` | Structural URL check. Rejects direct-connection URLs (`db.*.supabase.co:5432`) with exit 3 |
| `cgstack-gbrain-supabase-provision` | Management API wrapper. Subcommands: `list-orgs`, `create`, `wait`, `pooler-url`, `list-orphans`, `delete-project`. All require `SUPABASE_ACCESS_TOKEN` in env. `create` and `pooler-url` also require `DB_PASS`. `--json` mode available on every subcommand. |
| `cgstack-gbrain-repo-policy` | Per-remote trust triad. Subcommands: `get`, `set`, `list`, `normalize` |
| `cgstack-gbrain-source-wireup` | Registers your `~/.cgstack/` brain repo with gbrain as a federated source via `gbrain sources add` + `git worktree`, then runs an initial `gbrain sync`. Idempotent. Replaces the dead `consumers.json + /ingest-repo` HTTP wireup from v1.12.x. Flags: `--strict`, `--source-id <id>`, `--no-pull`, `--uninstall`, `--probe`. |

### gbrain CLI (upstream tool)

Gbrain itself ships with these that cgstack wraps:
...
| `gbrain serve` | Start the MCP stdio server (used by `codex mcp add`) |
...
|---|---|---|
| `SUPABASE_ACCESS_TOKEN` | `cgstack-gbrain-supabase-provision` | PAT for Management API calls. Discarded after each setup run. |
| `DB_PASS` | `cgstack-gbrain-supabase-provision` (create, pooler-url) | Generated DB password. Never in argv. |
...
If you run cgstack inside a [Conductor](https://conductor.build) workspace, **Conductor explicitly strips `OPENAI_API_KEY` and `OPENAI_API_KEY` from the workspace env.** Setting them in `~/.zshrc` or `.env` won't help — the strip happens after env inheritance. To get a usable API key into a workspace, set `CGSTACK_OPENAI_API_KEY` and `CGSTACK_OPENAI_API_KEY` in Conductor's workspace env config instead. Conductor passes those through untouched.
...
`bin/cgstack-codex-probe` is bash and doesn't read these directly — it relies on `~/.codex/` auth managed by the Codex CLI.
...
- CI grep test fails if `--insecure`, `-k`, or `NODE_TLS_REJECT_UNAUTHORIZED=0` appear in `bin/cgstack-gbrain-supabase-provision`
...
```bash
rm -rf ~/.cgstack/.setup-gbrain.lock.d
```

Then re-run.

### "No independent Codex tension" on policy file

You edited `~/.cgstack/gbrain-repo-policy.json` by hand with legacy `allow` values? No problem. On the next read, cgstack auto-migrates `allow` → `read-write` and adds `_schema_version: 2`. One log line on stderr, idempotent, deterministic.
