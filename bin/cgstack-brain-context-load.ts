#!/usr/bin/env bun
/**
 * cgstack-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the cgstack preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   cgstack-brain-context-load --skill office-hours --repo garrytan-cgstack
 *   cgstack-brain-context-load --skill-file ./SKILL.md --repo X --user Y
 *   cgstack-brain-context-load --window 14d --explain
 *   cgstack-brain-context-load --quiet
 */
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/cgstack-memory-helpers";
...
function printUsage(): void {
  console.error(`Usage: cgstack-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common cgstack skill locations
  const candidates = [
    join(HOME, ".codex", "skills", args.skill, "SKILL.md"),
    join(HOME, ".codex", "skills", "cgstack", args.skill, "SKILL.md"),
    join(process.cwd(), ".codex", "skills", args.skill, "SKILL.md"),
...
main().catch((err) => {
  console.error(`cgstack-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
