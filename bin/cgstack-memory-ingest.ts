#!/usr/bin/env bun
/**
 * cgstack-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.cgstack/ curated artifacts and writes
 ...
 *
 * Usage:
 *   cgstack-memory-ingest --probe                 # count what would ingest, no writes
 *   cgstack-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   cgstack-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   cgstack-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   cgstack-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 *   ~/.codex/projects/<encoded-cwd>/<uuid>.jsonl    — Codex sessions
 *   ~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl    — Codex CLI sessions
 *   ~/.cgstack/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.cgstack/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.cgstack/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.cgstack/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.cgstack/analytics/eureka.jsonl                — typed: eureka
 *   ~/.cgstack/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.cgstack/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/cgstack-memory-helpers#secretScanFile (D19).
...
import {
  ...
  detectEngineTier,
  withErrorContext,
} from "../lib/cgstack-memory-helpers";
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (cgstack-brain-sync, git push)
...
const HOME = homedir();
const CGSTACK_HOME = process.env.CGSTACK_HOME || join(HOME, ".cgstack");
const STATE_PATH = join(CGSTACK_HOME, ".transcript-ingest-state.json");
...
function printUsage(): void {
  console.error(`Usage: cgstack-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; cgstack-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.CGSTACK_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.CGSTACK_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "cgstack-memory-ingest",
      sessions: {},
    };
  }
  try {
    ...
    if (parsed.schema_version !== 1) {
      ...
      return { schema_version: 1, last_writer: "cgstack-memory-ingest", sessions: {} };
    }
    return parsed;
  } catch (err) {
...
function saveState(state: IngestState): void {
  ...
  // in cgstack-gbrain-sync.ts:saveSyncState.
  try {
...
function* walkCodexProjects(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const root = join(HOME, ".codex", "projects");
...
function* walkGstackArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(CGSTACK_HOME, "projects");

  // Eureka log: ~/.cgstack/analytics/eureka.jsonl
  const eurekaLog = join(CGSTACK_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.cgstack/builder-profile.jsonl
  const builderProfile = join(CGSTACK_HOME, "builder-profile.jsonl");
...
function parseTranscriptJsonl(path: string): ParsedSession | null {
  ...
  // Detect Codex rollout/session formats.
  const first = parsedLines[0];
  const isRollout = first?.type === "session_meta" || first?.payload?.id != null;
  const agent: "codex" = "codex";
  ...
  if (isRollout) {
    ...
    // Codex project JSONL: look for cwd in the first record that carries it.
  ...
  for (const rec of parsedLines) {
    ...
    } else if (isRollout && rec?.payload?.message) {
      // Codex rollout shape: each record has payload.message
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.cgstack/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.cgstack\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
...
function renderPageBody(page: PageRecord): string {
  ...
  // Strip NUL bytes — Postgres rejects 0x00 in UTF-8 text columns. Some Codex
...
interface PreparedPage {
  /** Page slug (path-shaped, e.g. "transcriptsCodex CLI/foo"). */
  slug: string;
  /** Original source file on disk (e.g. ~/.codex/projects/.../foo.jsonl). */
  source_path: string;
 ...
 * slashes (e.g. "transcriptsCodex CLI/foo") must live in matching
 ...
 *
 *   The actual cross-machine exfiltration boundary is `cgstack-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/cgstack-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because cgstack-brain-sync already gates the cross-machine boundary
...
/**
 * Make a per-run staging directory at ~/.cgstack/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(CGSTACK_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.cgstack/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `gbrain import`, remote-http users get a stable path that survives.
 * cgstack-brain-sync's allowlist pushes ~/.cgstack/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.cgstack/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
 ...
 *
 * Reads ~/.codex.json directly (same fallback chain as cgstack-gbrain-detect
 * Tier 3). Cheap: one fs read, no fork-exec.
 */
function isRemoteHttpMcpMode(): boolean {
  const home = process.env.HOME || homedir();
  const codexJsonPath = join(home, ".codex.json");
  if (!existsSync(codexJsonPath)) return false;
  try {
    const parsed = JSON.parse(readFileSync(codexJsonPath, "utf-8")) as {
      mcpServers?: {
...
function runGbrainImport(
  ...
  return new Promise((resolve) => {
    ...
    // parent cgstack-gbrain-sync seeding the bun grandchild's env).
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "cgstack-memory-ingest";
    saveState(state);
    return {
      written,
  ...
  // PERSISTENT dir under ~/.cgstack/transcripts/ and SKIP `gbrain import`
  // entirely. cgstack-brain-sync push will pick the dir up via its allowlist
  ...
  try {
    ...
    // staged markdown lives under ~/.cgstack/transcripts/<run-id>/ and the
    // next cgstack-brain-sync push will move it to the artifacts repo. From
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "cgstack-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
...
main().catch((err) => {
  console.error(`cgstack-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
