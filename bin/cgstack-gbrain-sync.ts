#!/usr/bin/env bun
/**
 * cgstack-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → cgstack-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → cgstack-brain-sync (existing pipeline)
 *
 * Modes:
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/cgstack-memory-helpers";
...
const STATE_PATH = join(CGSTACK_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(CGSTACK_HOME, ".sync-gbrain.lock");
const STALE_LOCK_MS = 5 * 60 * 1000;

// ── CLI ────────────────────────────────────────────────────────────────────

function printUsage(): void {
  console.error(`Usage: cgstack-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the cgstack-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the cgstack-brain-sync git pipeline stage.
 ...
 *
 * Pattern: `cgstack-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `CGSTACK_HOSTNAME` env override is honored for deterministic tests; in
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.CGSTACK_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("cgstack-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("cgstack-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cgstack-code", `${base}-${hostPathHash}`);
}
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("cgstack-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cgstack-code", base);
}
 ...
 * repo path: `cgstack-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("cgstack-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("cgstack-code", `${base}-${pathHash}`);
}
...
function acquireLock(): boolean {
  mkdirSync(CGSTACK_HOME, { recursive: true });
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  // cgstack's own .gitignore. Without the consumer-side entry, the pin gets
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: cgstack-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). cgstack-memory-ingest shells out
  ...
  const ingestPath = join(import.meta.dir, "cgstack-memory-ingest.ts");
  ...
  // .env.local footgun affects cgstack-memory-ingest.ts too, not just the
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: cgstack-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "cgstack-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (cgstack-brain-sync not installed)" };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `cgstack-brain-sync exited ${result.status}`,
  };
}
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "cgstack-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "cgstack-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "cgstack-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "cgstack-gbrain-sync"));
    }
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`cgstack-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
