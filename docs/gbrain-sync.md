...
cgstack writes a lot of useful state to `~/.cgstack/` — learnings, retros, CEO
...
- A clean off-ramp (`cgstack-brain-uninstall`) that never touches your data.
...
The exact allowlist lives in `~/.cgstack/.brain-allowlist`. The CLI manages
...
1. Turns `~/.cgstack/` into a git repo.
2. Asks for a remote URL (default: `gh repo create --private
   cgstack-brain-$USER`). Any git remote works — GitHub, GitLab, Gitea,
   self-hosted.
3. Pushes an initial commit with just the config.
4. Writes `~/.cgstack-brain-remote.txt` (URL-only, no secrets —
   safe to copy to another machine).
5. Wires the cgstack-brain repo into your local gbrain as a federated
   ...
   lives in `bin/cgstack-gbrain-source-wireup`. The old
   `cgstack-brain-reader add --ingest-url ...` HTTP path was removed in
...
- **Decline**: keep everything local. You can turn sync on later with
  `cgstack-config set artifacts_sync_mode full`.

Your answer is persisted. You won't be asked again.

## Cross-machine workflow

On machine A: run `cgstack-brain-init` once. That's it — every skill
invocation now drains the sync queue at its start and end boundaries
(~200–800 ms network pause per skill).

On machine B:

1. Copy `~/.cgstack-brain-remote.txt` from machine A to machine B
   (password manager, dotfile repo, USB stick — your call).
2. Run any cgstack skill. The preamble sees the URL file and prints:
   ```
   BRAIN_SYNC: brain repo detected: <url>
   BRAIN_SYNC: run 'cgstack-brain-restore' to pull your cross-machine memory
   ```
3. Run `cgstack-brain-restore`. That clones the repo, rehydrates your
...
4. Re-enter consumer tokens (they're machine-local and NOT synced —
   `cgstack-config set gbrain_token <your-token>`).
...
```bash
cgstack-config set artifacts_sync_mode full
cgstack-config set artifacts_sync_mode off
```
...
2. If the match is a false positive on content you explicitly want to
   sync, run `cgstack-brain-sync --skip-file <path>` to permanently
   exclude that path.
3. Otherwise, edit the file to remove the secret and re-run any skill.

There's a defense-in-depth hook at `~/.cgstack/.git/hooks/pre-commit` that
...
(cached via `~/.cgstack/.brain-last-pull`). You don't need to think about
this — it happens automatically at the first skill invocation each day.

## Uninstall

```bash
cgstack-brain-uninstall
```

This:

- Removes `~/.cgstack/.git/` and all `.brain-*` config files.
- Clears `artifacts_sync_mode` in `cgstack-config`.
...
Re-init anytime with `cgstack-brain-init`.

## Troubleshooting

See [gbrain-sync-errors.md](gbrain-sync-errors.md) for an index of every
error message cgstack-brain may print, with problem / cause / fix for each.
