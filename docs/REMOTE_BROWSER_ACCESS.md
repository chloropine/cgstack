...
A CGStack Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
CGStack Browser Server                 Any AI agent
  ├── Chromium (Playwright)           (Codex, Codex, Codex, etc.)
...
3. **User copies** the instruction block into the other Codex session's chat
