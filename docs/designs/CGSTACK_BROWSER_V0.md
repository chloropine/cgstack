# CGStack Browser V0 — The AI-Native Development Browser

**Date:** 2026-03-30
**Author:** Garry Tan + Codex
**Status:** Phase 1a shipped, Phase 1b in progress
**Branch:** chloropine/cgstack-as-browser

## The Thesis

Every other Codex browser (Atlas, Dia, Comet, Chrome Auto Browse) starts with a
consumer browser and bolts AI onto it. CGStack Browser inverts this. It starts
with Codex as the runtime and gives it a browser viewport.
...
lives in the browser. The AI works across both simultaneously. What AI editors did
for text editors, CGStack Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the cgstack
sidebar extension baked in. You open it and Codex can see your screen,
navigate pages, fill forms, take screenshots, inspect CSS, clean up overlays,
and run any cgstack skill. All without touching a terminal.

```
CGStack Browser.app (389MB, 189MB DMG)
...
`codex -p` subprocess. BoomLooper provides:
...
When the extension side panel hits hard API limits, CGStack Browser ships to
...
Every cgstack skill becomes a browser capability.
...
+-----------┼────────────────────────────┼─────────────────+
  ...
  |  (HTTP + SSE)       |    |  (codex -p wrapper)  |
  |  :34567             |    |  Runs cgstack skills   |
  |                     |    |  Per-tab isolation     |
  |  Commands:          |    |                       |
...
| **AI code editors** | Editor + AI | Strong code editing | No browser viewport |
| **CGStack Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

CGStack Browser doesn't compete with consumer browsers. It competes with the
...
| `CGSTACK_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
| Model routing | **SHIPPED** | GPT for actions, GPT for analysis (`pickSidebarModel`) |
...
Local codex -p agent         Team workspaces                Cross-platform
...
The 12-month ideal: you open CGStack Browser, it detects your project, starts
...
Independent Codex review works.
