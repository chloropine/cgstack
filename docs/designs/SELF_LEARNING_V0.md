# Design: CGStack Self-Learning Infrastructure
...
CGStack runs 30+ skills across sessions but learns nothing between them. A $review
...
Every AI coding tool has this problem. Some tools have per-user memory. Codex has
...
Structured, typed, confidence-scored learnings that every cgstack skill can read and
write. The goal: after 20 sessions on the same codebase, cgstack knows every
...
YC founders building with AI. The people who run cgstack on real codebases 20+ times
...
|------|-------------|-------|-----------|
| AI code editors | Per-user chat memory | Per-session | Unstructured |
| AGENTS.md | Static file | Per-project | Manual |
| Windsurf | Persistent context | Per-session | Unstructured |
| **CGStack** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

cgstack has four distinct persistence layers. They share storage patterns
(JSONL in `~/.cgstack/projects/$SLUG/`) but serve different purposes:
...
| **Checkpoints** | `checkpoints/*.md` | Working state snapshots (decisions, remaining work, files) | $context-save, $ship, $investigate | Preamble context recovery, $context-restore |
...
### Release 1: "CGStack Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.cgstack/projects/{slug}/learnings.jsonl`
...
- Integration into $review, $ship, $plan-* skills, $office-hours, $investigate, $retro
...
Sources: `observed` | `user-stated` | `inferred` | `independent Codex`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via cgstack-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read cgstack-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via cgstack-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.cgstack/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), cgstack suggests what you probably want next.
- "Welcome back" synthesized context message on session start.
- `$context-save` / `$context-restore` skills: save/resume/list working state snapshots. Cross-branch
...
- Timeline binaries: `bin/cgstack-timeline-log` and `bin/cgstack-timeline-read`.
- Routing rules: $context-save, $context-restore, and $health added to preamble skill routing.

Design doc: `docs/designs/SESSION_INTELLIGENCE.md`

### Release 4: "Adaptive Ceremony" — NOT YET SHIPPED

**Headline:** CGStack respects your time without compromising your safety.
...
- Cross-platform portability (Copilot, Codex, Windsurf output)
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of CGStack's approach. We adapted every concept to fit CGStack's template system, voice, and architecture rather than porting directly.
