...
`/Users/garrytan/.codex/plans/purrfect-tumbling-quiche.md`:
...
exfiltration boundary is `cgstack-brain-sync` (bin/cgstack-brain-sync:78-110,
...
for 5031). That's a gbrain-side perf issue, not cgstack architecture.
...
1. **gbrain import perf on large dirs** — investigate why 5031 files
   ...
   FTS index updates without batching. Lives in gbrain, not cgstack.
...
1. **Subprocess-per-file architecture.** The ingest loop at line 911 walks
   1,841 files in `~/.cgstack/projects/` and spawns two subprocesses per file:
   - `gitleaks detect --no-git --source <path>` — 46ms cold start (`lib/cgstack-memory-helpers.ts:157`)
   - `gbrain put <slug>` — 329ms cold start (`bin/cgstack-memory-ingest.ts:823`)
   - Per-file floor: 375ms × 1841 = 690s (11.5 min) of pure subprocess startup
     before any actual work happens.

2. **Kill-no-save timeout.** Orchestrator at `bin/cgstack-gbrain-sync.ts:442`
   enforces a 35-min timeout. When it fires, `spawnSync` returns
   `result.status === null`, the child gets SIGTERM, and the in-memory
   ingest state never flushes to `~/.cgstack/.transcript-ingest-state.json`.
...
| Metric | Value | Source |
|---|---|---|
| Files in walkAllSources | 1,841 | `find ~/.cgstack/projects -type f \( -name "*.md" -o -name "*.jsonl" \)` |
...
Take everything in `ingestPass` (lines 899-988 of `bin/cgstack-memory-ingest.ts`)
...
- Created at `~/.cgstack/.staging-ingest-${pid}-${ts}/`
...
redaction policy in `lib/cgstack-memory-helpers.ts`).
...
### Step 6: SIGTERM handler in `cgstack-memory-ingest.ts`
...
In `bin/cgstack-gbrain-sync.ts:444`:
...
`~/.cgstack/.ingest-skip-list.json` so the next prepare stage skips that file
entirely. Avoids re-staging a file that will always fail. User can review
the skip list with a new `cgstack-memory-ingest --skip-list` flag.
...
3. **Benchmark gate (periodic, paid):**
   ...
   - Test fixture: copy of `~/.cgstack/projects/` snapshot for repeatable timing.

## Rollback strategy

- New `--legacy-ingest` flag on `cgstack-memory-ingest` keeps the old
  per-file path callable for one release cycle.
- If batch path regresses on a real corpus, set
  `cgstack-config set memory_ingest_path legacy` to revert without redeploy.
...
6. **Should we deprecate `gbrain put` for memory entirely?** The legacy
   ...
   ingestion? Probably yes (for `~/.cgstack/.transcript-ingest-state.json`
   updates triggered outside the orchestrator), but worth confirming.

## What this isn't

- Not a gbrain CLI change. All work is in cgstack.
- Not a AGENTS.md voice/UX change.
