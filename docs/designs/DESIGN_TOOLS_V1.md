# Design: cgstack Visual Design Generation (`design` binary)
...
5. Codex generates HTML wireframe via DESIGN_SKETCH matching approved direction
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.cgstack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/cgstack-mockups-{session}/` (ephemeral, not committed)
...
- Always show to user via the Read tool (which renders images inline in Codex)
- This avoids repo bloat: only approved designs are committed, not every exploration variant
- Fallback: if not in a git repo, save to `/tmp/cgstack-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `CGSTACK_DESIGN` env var, similar to how `CGSTACK_BROWSE` is set.
...
[ -z "$D" ] && D=~/.codex/skills/cgstack/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `CGSTACK_DESIGN` env var export for Codex host |
| `test/gen-skill-docs.test.ts` | Update DESIGN_SKETCH test suite for new resolvers |
| `setup` | Add design binary build + Codex/Codex asset linking |
...
1. Read `~/.cgstack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.cgstack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.codex/skills/cgstack/` install path
...
- Auth module (read ~/.cgstack/openai.json, fallback to env var, guided setup flow)
...
- `setup` script integration (including Codex + Codex asset linking)
...
- Add CGSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
...
- Outside voice: 4 passes (Codex structured 12 issues, Codex structured 8 issues, Codex adversarial 1 fatal flaw, Codex adversarial 1 fatal flaw). Key insight: sequential PNG→HTML workflow resolved the "opaque raster" fatal flaw.
