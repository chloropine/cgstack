/**
 * cgstack-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/cgstack-memory-ingest.ts (Lane A)
 *   - bin/cgstack-gbrain-sync.ts   (Lane B)
 *   - bin/cgstack-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.cgstack/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/chloropine/cgstack.git → github.com/chloropine/cgstack
 *   git@github.com:chloropine/cgstack.git     → github.com/chloropine/cgstack
 ...
 * Replaces the inadequate regex scanner in bin/cgstack-brain-sync (which only
...
function cgstackHome(): string {
  return process.env.CGSTACK_HOME || join(homedir(), ".cgstack");
}

function engineCachePath(): string {
  return join(cgstackHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(cgstackHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.cgstack/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "cgstack-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.cgstack/.gbrain-errors.jsonl. Never throws.
 ...
 * runtime read path used by cgstack-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(cgstackHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.cgstack/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
