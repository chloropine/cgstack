---
name: open-cgstack-browser
version: 0.2.0
description: |
  Launch CGStack Browser — AI-controlled Chromium with the sidebar extension baked in.
  Opens a visible browser window where you can watch every action in real time.
  The sidebar shows a live activity feed and chat. Anti-bot stealth built in.
  Use when asked to "open cgstack browser", "launch browser", "connect chrome",
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-cgstack-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.cgstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.codex/skills/cgstack/bin/cgstack-timeline-log '{"skill":"open-cgstack-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.codex/skills/cgstack/bin/cgstack-question-log '{"skill":"open-cgstack-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# $open-cgstack-browser — Launch CGStack Browser

Launch CGStack Browser — AI-controlled Chromium with the sidebar extension,
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.cgstack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.cgstack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.cgstack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.cgstack/chromium-profile"
...
This launches CGStack Browser (rebranded Chromium) in headed mode with:
- A visible window you can watch (not your regular Chrome — it stays untouched)
- The cgstack sidebar extension auto-loaded via `launchPersistentContext`
- Anti-bot stealth patches (sites like Google and NYTimes work without captchas)
- Custom user agent and CGStack Browser branding in Dock/menu bar
- A sidebar agent process for chat commands

The `connect` command auto-discovers the extension from the cgstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.cgstack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.agents/skills/cgstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.agents/skills/cgstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.codex/skills/cgstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.codex/skills/cgstack/extension"
...
> Chrome is launched with cgstack control. You should see Playwright's Chromium
...
>    already show the cgstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **cgstack browse** → click the **pin icon**
> 3. Click the pinned **cgstack icon** in the toolbar
...
> 2. Look for **"cgstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the cgstack icon
...
> - Run any cgstack skill (`$qa`, `$design-review`, `$benchmark`) and watch
